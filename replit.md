...
- Газ/жд/связь/объекты наследия теперь определяются и в демо-режиме (полосы инфраструктуры поверх участков), и в реальном режиме через отдельный запрос к слою OKS Росреестра — но в реальном режиме тип объекта распознаётся эвристически по ключевым словам в названии/адресе (официальной схемы атрибутов у неофициального API нет), это best-effort, а не гарантированная классификация
