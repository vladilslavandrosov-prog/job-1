...
Весь сайт (кроме `/api/v1/health`) закрыт формой входа (`/login`) на одного
тестового пользователя — это демо-заглушка, а не полноценная
многопользовательская система. Логин/пароль задаются переменными окружения
`AUTH_USERNAME` / `AUTH_PASSWORD`; без них используются дефолты для локальной
разработки (`demo` / `demo2026`) — **обязательно переопределите их на реальном
деплое**. Сессия — обычная подписанная Flask-cookie (`SECRET_KEY`), живёт 7 дней.
